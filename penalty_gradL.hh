template<class GV>
void penalty_gradL(const GV& gv, Dune::BlockVector<Dune::FieldVector<double,1>>& gradL1, Dune::BlockVector<Dune::FieldVector<double,1>>& u_i, Dune::BlockVector<Dune::FieldVector<double,1>>& phi_i, Dune::BlockVector<Dune::FieldVector<double,1>>& atom_node, double side_length, double penalty, double integral_u)
{
	...
	Dune::BlockVector<Dune::FieldVector<double,1>> gradNi(dim); 
	Dune::BlockVector<Dune::FieldVector<double,1>> gradu(dim);
	Dune::BlockVector<Dune::FieldVector<double,1>> gradphi(dim);
	double C_F, Ts1, Ts2, Ts, L1, L2, ES1, ES2, Exc, u, phi, eps_x, gradeps_x, eps_c, gradeps_c, sum, sum1, rs, deriv_rs, ep_interaction;
	...
	{				
	  	...
	  	phi = 0.0;	
			...
			for (int i = 0 ; i<vertexsize; i++)
			{
			  ...
			  gradphi += gradN;		
				...
				deriv_rs = (pow((3/(4*pi)),0.3333333333333333))*(-2./3.)*pow(std::abs(u),-1.66666667); //((pow(u,-3))*(pow(pow(u,4),0.333333333333)));	
   			eps_x = (-3./4.)*pow((3./pi),0.3333333)*pow(std::abs(u),0.666667) ;//(pow(pow(u,2),0.3333333333));
				//std::cout << "rs  = " << rs <<  "deriv_rs  = " << deriv_rs << "\n" ;
        ...
        // Gradient of the Exchange-Coorelation Term
  			gradeps_x = (-0.5)*pow((3./pi),0.33333333)*pow(std::abs(u),-0.3333333);//*u*(pow(pow(u,-4),0.3333333333));
				if(rs > 1.0 || rs == 1.0)
				{
			...
			for (int i = 0 ; i<vertexsize; i++)
			{
				...
				/*gradL1[set.subIndex(*el,i,dim)] += (C_F*(10./3.)*(((pow(pow(u,4),0.3333333333))*u)*basis[i].evaluateFunction(r->position())) + lam_kin*(gradu*gradNi) + ((gradeps_x)*u + (gradeps_c)*pow(u,2) + 2*(eps_x + eps_c)*u)*basis[i].evaluateFunction(r->position()) + (2.0*u*phi*basis[i].evaluateFunction(r->position())) + 4.0*penalty*(integral_u - NegativeCharge)*u*basis[i].evaluateFunction(r->position()))*weight*detjac; */

				gradL1[set.subIndex(*el,i,dim)] += (C_F*(10./3.)*((pow(std::abs(u),2.3333333))*basis[i].evaluateFunction(r->position())) 
			+ lam_kin*(gradu*gradNi) + ((gradeps_x)*u + (gradeps_c)*pow(u,2) 
			+ 2*(eps_x + eps_c)*u)*basis[i].evaluateFunction(r->position()) 
			+ (2.0*u*phi*basis[i].evaluateFunction(r->position())) 
			/*+ (phi - ep_interaction)*u*basis[i].evaluateFunction(r->position())*/
			+ 4.0*penalty*(integral_u - NegativeCharge)*u*basis[i].evaluateFunction(r->position()))*weight*detjac; 
	...
	{
		gradL1[boundary_nodes[count]] = 0.0;
	}
