template<class GV>
void lagrange_gradLphi(const GV& gv, Dune::BlockVector<Dune::FieldVector<double,1>>& gradLphi, Dune::BlockVector<Dune::FieldVector<double,1> >& u_i, Dune::BlockVector<Dune::FieldVector<double,1> >& phi_i, Dune::BlockVector<Dune::FieldVector<double,1>>& atom_node, double side_length, double lagrange, double integral_u)
{
	static const int dim = 3;
	typedef Dune::UGGrid<dim>::ctype ctype;
	const int N = gv.size(dim);
	Dune::BlockVector<Dune::FieldVector<double,1> > x(N), gradN(dim), gradu(dim), gradphi(dim), gradL(N);
	double C_F, Ts1, Ts2, Ts, L1, L2, ES1, ES2, Exc, u, phi, sum, sum1, eps_x, rs, eps_c;
  ...
  typedef typename GV::IndexSet LeafIndexSet;
	...
	C_F = 0.3*pow((3*pi*pi),0.66666666666);
		...
		double penalty = 1.0; //set arbitrarily
	...
	// Iteration over elements to compute the gradLphi
...
/*	for (ElementLeafIterator el = gv.template begin<0>(); el != elend; ++el)
	{
		...
		for (typename Dune::QuadratureRule<ctype,dim>::const_iterator r = rule2.begin();r != rule2.end() ; ++r)
		{
			...
			{
			jacInvTra.mv(basis[i].evaluateGradient(r->position()),gradN);		     			
			gradLphi[set.subIndex(*el,i,dim)] += ( (-1/(4*pi))*(gradphi*gradN)*weight*detjac + (pow(u,2)*basis[i].evaluateFunction(r->position()))*weight*detjac);
			//gradLphi[set.subIndex(*el,i,dim)] += (pow(u,2)*basis[i].evaluateFunction(r->position()))*weight*detjac;
			}
	...
	//gradLphi[atom_node] += -Ncore;
	// Set gradient to zero in the 
	
	watch_GridIter.reset();
	for (int count = 0; count < total_count; count++)
	{
		gradLphi[boundary_nodes[count]] = 0.0;
	} 
