template<class GV>
void penalty_energy(const GV& gv, Dune::BlockVector<Dune::FieldVector<double,1>>& u_i, Dune::BlockVector<Dune::FieldVector<double,1>>& phi_i, Dune::BlockVector<Dune::FieldVector<double,1>>& atom_node, double penalty, double lagrange, double integral_u, double& E, double& E1, double& E_penalty, double& E_lagrange, double& Enucnuc, const std::list<ATOM> atoms)
{
  ...
  int vertexsize = ref.size(dim);
	...
	{		
		...
		//	std::cout << "ep_interaction" << ep_interaction <<"\n";
		  ...
		  Ts = Ts1 + 0.5*lam_kin*Ts2; 
			...
			E = Ts + Exc + ES1 + ES2 + E_penalty + Enucnuc;  
		  E1 = Ts + Exc + L1 + L2 + E_penalty + Enucnuc; 
